...
%len= length(XX); % not needed for loc (commente by Ricardo Vázquez)
r= z(1); b= z(2);
s= sin(XX(3)+b); 
c= cos(XX(3)+b);
...
% P augmentation not needed, only state augmentation is used to store the
% map (modified by Ricardo Vázquez for EKF-localization version)
